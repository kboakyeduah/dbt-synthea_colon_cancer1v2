{% set column_names = 
    dbt_utils.get_filtered_columns_in_relation( source('staging', 'claims_transactions') ) 
%}
...
WITH cte_claims_transactions_lower AS (
    ...
    FROM {{ source('staging','claims_transactions') }}
)
...
, cte_claims_transactions_rename AS (
    ...
    SELECT
        ...
        , CAST(claimid AS UUID) AS claim_id
        ...
        , CAST(placeofservice AS UUID) AS place_of_service
        ...
        , CAST(supervisingproviderid AS UUID) AS supervising_provider_id
