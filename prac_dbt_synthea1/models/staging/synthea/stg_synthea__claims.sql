{% set column_names = 
    dbt_utils.get_filtered_columns_in_relation( source('staging', 'claims') ) 
%}
...
, cte_claims_rename AS (
    ...
    SELECT
        ...
        , CASE
    WHEN CAST(primarypatientinsuranceid AS VARCHAR) = '0' THEN NULL
    ELSE CAST(primarypatientinsuranceid AS UUID)
  END AS primary_patient_insurance_id
        , CASE
    WHEN CAST(secondarypatientinsuranceid AS VARCHAR) = '0' THEN NULL
    ELSE CAST(secondarypatientinsuranceid AS UUID)
